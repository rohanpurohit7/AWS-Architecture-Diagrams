...
def api_get(url: str, authenticated: bool = True) -> Any:
    ...
    try:
        ...
        # A repository-scoped GITHUB_TOKEN can return 403/404 for other public
        # repositories. Retry those reads anonymously before reporting yellow.
...
def failed_steps(repo_name: str, run_id: int) -> list[dict[str, str]]:
    ...
    for job in payload.get("jobs", []):
        ...
        failed = [
            ...
            if (step.get("conclusion") or "") in FAILURE_CONCLUSIONS
        ]
...
def write_markdown(data: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        f"Last refreshed: `{data['generated_at']}`",
        "",
    ...
    for repo in data["repositories"]:
        ...
        if repo.get("run_url"):
            workflow = f"[{repo.get('workflow') or 'workflow'}]({repo['run_url']})"
        else:
            workflow = repo.get("workflow") or "—"
        lines.append(
            "| {icon} **{signal}** | {repo} | {workflow} | {detail} | {branch} | {updated} |".format(
                icon=ICONS[repo["signal"]],
                signal=repo["signal"],
                repo=repo_link,
                workflow=workflow,
                detail=repo.get("detail") or "—",
    ...
    if not data["errors"]:
        ...
        for error in data["errors"]:
            ...
            if steps:
                ...
                for step in steps:
                    lines.append(
                        f"| {step['job']} | {step['step']} | `{step['conclusion']}` |"
                    )
            else:
...
def main() -> int:
    ...
    try:
        data = build()
    except Exception as exc:  # top-level failure should be explicit in Actions
        print(f"DevOps signal generation failed: {exc}", file=sys.stderr)
        return 1
    STATUS_JSON.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
    write_markdown(data)
    print(
        "DevOps signal updated: "
        f"green={data['summary']['green']} "
        f"yellow={data['summary']['yellow']} "
        f"red={data['summary']['red']}"
    )
    return 0
